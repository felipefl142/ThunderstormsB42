...
function ThunderClient.DoStrike(args)
    ...
    brightness = math.max(0.1, math.min(0.5, brightness))

    DebugPrint("  Base brightness: " .. string.format("%.2f", brightness))

    -- Queue multi-flash sequence
    ThunderClient.flashSequence = {}
    local numFlashes = ZombRand(1, 4)
    local now = getTimestampMs()
    local cumulativeDelay = 0

    DebugPrint("  Queuing " .. numFlashes .. " flash(es)")
    ...
    for i = 1, numFlashes do
        ...
        local flashIntensity = brightness * (ZombRandFloat(0.75, 1.25))
